...
jest.mock('../util/result-set-mapper', () => {
    return {
        mapUserResultSet2: jest.fn()
    };
});

describe('orderLineRepo', () => {

    let sut = new OrderLineRepository();
    ...
    beforeEach(() => {
        ...
        */
        ...
        (mockMapper.mapUserResultSet2 as jest.Mock).mockClear();
    });
    ...
    test('should resolve to an array of Order when getAll retrieves records from data source', async () => {
        ...
        let mockOrder = new OrderLine(1, 1, 4);
        (mockMapper.mapUserResultSet2 as jest.Mock).mockReturnValue(mockOrder);
    ...
    test('should resolve to a Order object when getById retrieves a record from data source', async () => {
        ...
        let mockOrder = new OrderLine(1, 2, 4);
        (mockMapper.mapUserResultSet2 as jest.Mock).mockReturnValue(mockOrder);

        // Act
        let result = await sut.getById();

        // Assert
        expect(result).toBeTruthy();
        expect(result instanceof OrderLine).toBe(true);
